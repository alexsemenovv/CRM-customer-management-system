...
class StatisticTestCase(TestCase):
    ...
    def test_check_amount_products(self):
        ...
        response = self.client.post(reverse("statisticsapp:index"))
        self.assertEqual(response.status_code, 200)
        amount_products = len(
            response.context["products"]
        )  # получаем кол-во услуг в БД
    ...
    def test_check_amount_leads(self):
        ...
        amount_leads_plus_one = len(
            response.context["leads"]
        )  # получаем кол-во услуг в БД
        self.assertEqual(response.status_code, 200)
    ...
    def test_check_amount_customers(self):
        ...
        response = self.client.post(reverse("statisticsapp:index"))
        self.assertEqual(response.status_code, 200)
        amount_customers = len(
            response.context["customers"]
        )  # получаем кол-во клиентов в БД

        new_customer = self.client.post(
            reverse("customersapp:customers_create"), {"lead": 8, "contract": 10}
